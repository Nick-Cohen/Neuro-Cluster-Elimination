...
class FactorNN(FastFactor):
    ...
    def _get_slices(self, assignments, elim_vars, elim_domain_sizes, message_scope):
        ...
        if self.gm.lower_dim:
            ...
            # Full one-hot encoding: include all categories
            one_hot_encoded_samples = torch.cat([F.one_hot(nn_assignments[:, i], num_classes=self.domain_sizes[i]) for i in range(len(self.labels))], dim=-1)
    ...
    def nn_to_FastFactor(fastGM, jit_file = None, net = None, device='cuda', debug=False, data_processor=None):
        ...
        if fastGM.lower_dim:
            ...
            # Full one-hot encoding: include all categories
            total_inputs_full = domain_sizes.sum().item()  # Recalculate total for full encoding
            all_inputs = torch.zeros((assignments.shape[0], total_inputs_full), device=device)
            
            offset = 0
            for i, size in enumerate(domain_sizes):
                if size > 1:
                    mask = assignments[:, i].unsqueeze(1) == torch.arange(0, size, device=device)  # Start from 0
                    all_inputs[:, offset:offset+size] = mask.float()
                    offset += size
                else:
